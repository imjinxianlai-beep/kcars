...
export function generateInvoicePDF(invoice, customer, items, invoiceType, logoImage = null) {
  ...
  if (co.hasLogo) {
    ...
    if (logoImage?.dataUrl && logoImage.width && logoImage.height) {
      const targetH = 28
      const targetW = targetH * (logoImage.width / logoImage.height)
      doc.addImage(logoImage.dataUrl, 'PNG', M, y, targetW, targetH)
    }
  ...
  // Total label (left of box) + number box (right)
  const totalBoxW = 28
  const totalLabelW = 22
  const totalBoxX = W - M - totalBoxW
  const totalLabelX = totalBoxX - totalLabelW - 2
  doc.setFont('helvetica','bold').setFontSize(9.5).setTextColor(20)
  doc.text(co.totalLabel, totalLabelX + totalLabelW, y+5.5, {align:'right'})
  doc.setDrawColor(120).setLineWidth(0.4)
  doc.rect(totalBoxX, y, totalBoxW, 8)
  doc.setFont('helvetica','bold').setFontSize(10).setTextColor(20)
  doc.text(total.toFixed(2), W-M-1, y+5.8, {align:'right'})
  y += 13
...
export async function downloadInvoice(invoice, customer, items, type) {
  const invoiceType = type || detectInvoiceType(items)
  const logoImage = invoiceType.startsWith('kc') ? await loadLogoImage() : null
  const doc = generateInvoicePDF(invoice, customer, items, invoiceType, logoImage)
